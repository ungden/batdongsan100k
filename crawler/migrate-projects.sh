#!/bin/bash
SUPABASE_URL=$(grep NEXT_PUBLIC_SUPABASE_URL web/.env.local | cut -d '=' -f2)
SUPABASE_KEY=$(grep SUPABASE_SERVICE_ROLE_KEY web/.env.local | cut -d '=' -f2)

cat << JS > crawler/run-migrate.mjs
...
async function migrate() {
  ...
  for (const [slug, proj] of projectData.entries()) {
    ...
    let { data: existingProject } = await supabase.from('projects').select('id').eq('slug', slug).maybeSingle();
