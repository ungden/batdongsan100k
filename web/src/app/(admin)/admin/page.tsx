...
export default async function AdminDashboard() {
  ...
  const [
    ...
    supabase.from('properties').select('*', { count: 'estimated', head: true }).eq('status', 'published'),
    supabase.from('properties').select('*', { count: 'estimated', head: true }).eq('status', 'pending_review'),
